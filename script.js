        ...
        <div style="background: white; padding: 10px; border-radius: 15px;">
            ...
            <div style="display: grid; grid-template-columns: repeat(5, 1fr); gap: 4px;">
                ${card.map(num => {
                    ...
                    return `<div style="background:${isHit ? '#e94560' : '#1f4068'}; color:white; height:40px; display:flex; justify-content:center; align-items:center; font-weight:bold; border-radius:5px; font-size:0.8rem;">${num === "FREE" ? "FREE" : num}</div>`;
                }).join("")}
            </div>
        </div>
    `;
...
function callNumbers() {
    ...
    const inv = setInterval(() => {
        ...
        called.push(n);
        // እዚህ ጋር currentNum ላይ ፊደሉን ጨምረን እንልካለን
